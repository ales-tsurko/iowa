use iowa_compiler::runtime::runtime::{Runtime, Value, MethodBody};
...
fn test_wasm_runtime_object_interaction() {
    ...
    // Add the WASM method to the test object
    if let Some(obj) = runtime.memory_mut().get_object_mut(test_obj) {
        obj.set_slot("create_object_wasm".to_string(), Value::Object(wasm_method_id_alloc));
    }
    
    // Execute the WASM method
    let result_alloc = runtime.dispatch_message(Value::Object(test_obj), "create_object_wasm", vec![]);
...
fn test_wasm_runtime_argument_passing() {
    ...
    let method_id = runtime.create_method_with_wasm(
...
fn test_wasm_memory_management() {
    ...
    // The fact that it executes successfully means our memory management is working at a basic level
    assert_ne!(result, Value::Nil);
