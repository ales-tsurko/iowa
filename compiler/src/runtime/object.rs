...
pub fn get_prototype(builder: &mut FunctionBuilder, object: CraneliftValue) -> CraneliftValue {
    ...
    builder
        .ins()
        .brif(is_object, load_block, &[], nil_block, &[]);
...
pub fn lookup_slot(
    ...
    builder
        .ins()
        .brif(is_nil, prototype_block, &[], done_block, &[slot_value]);
...
fn lookup_slot_in_object(
    ...
    builder
        .ins()
        .brif(is_object, start_lookup_block, &[], not_object_block, &[]);
    ...
    let at_end = builder
        .ins()
        .icmp(IntCC::UnsignedGreaterThanOrEqual, curr_idx, capacity);
    builder.ins().brif(
        at_end,
        done_block,
        &[nil_value],
    ...
    builder
        .ins()
        .brif(is_empty, loop_next, &[entry_ptr, idx], name_match, &[]);
    ...
    let next_entry = builder
        .ins()
        .iadd_imm(curr_entry, memory::layout::slots_table::ENTRY_SIZE as i64);
...
pub fn set_slot(
    ...
    builder
        .ins()
        .brif(is_object, set_block, &[], done_block, &[value]);
    ...
    builder
        .ins()
        .jump(loop_header, &[entry_start, zero_idx, zero_ptr]);
    ...
    builder.ins().brif(
        ...
        &[first_empty],
        loop_body,
        &[curr_entry, curr_idx, first_empty],
    );
    ...
    builder
        .ins()
        .jump(loop_header, &[next_entry, next_idx, curr_empty]);
    ...
    builder
        .ins()
        .brif(has_empty, add_to_empty, &[], expand_table, &[]);
