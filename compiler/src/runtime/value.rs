...
pub const TAG_NUMBER: u64 = 2;
pub const TAG_OBJECT_REF: u64 = 3;
pub const TAG_STRING_REF: u64 = 4;
pub const TAG_MESSAGE_REF: u64 = 5;
...
pub fn encode_nil(builder: &mut FunctionBuilder) -> CraneliftValue {
    ...
    builder.ins().iconst(types::I64, TAG_NIL as i64)
}
...
pub fn encode_boolean(builder: &mut FunctionBuilder, value: bool) -> CraneliftValue {
    ...
    let shifted = builder.ins().ishl_imm(bool_val, TAG_BITS as i64);
    let tag = builder.ins().iconst(types::I64, TAG_BOOLEAN as i64);
...
pub fn encode_reference(
    ...
    let shifted = builder.ins().ishl_imm(ptr, TAG_BITS as i64);
...
pub fn has_tag(builder: &mut FunctionBuilder, value: CraneliftValue, tag: u64) -> CraneliftValue {
    let tag_value = decode_tag(builder, value);
    let tag_const = builder.ins().iconst(types::I64, tag as i64);
    builder.ins().icmp(
...
pub fn decode_reference(builder: &mut FunctionBuilder, value: CraneliftValue) -> CraneliftValue {
    builder.ins().ushr_imm(value, TAG_BITS as i64)
}
