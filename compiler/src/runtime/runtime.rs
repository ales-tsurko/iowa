...
impl Runtime {
    ...
    fn execute_method_body(&mut self, body: &MethodBody) -> Option<Value> {
        match body {
            MethodBody::Source(source) => match iowa_parser::parse(source) {
                Ok((_, chains)) if !chains.is_empty() => {
                    ...
                    let method = crate::runtime::wasm_runtime::WasmMethod::from_ast(
                        &chains[0],
                        &method_data,
                        self,
                    );
    ...
    fn execute_message_chain(&mut self, chain: &iowa_parser::MessageChain) -> Option<Value> {
        ...
        for message in &chain.messages {
                ...
                if id.name() == "if" {
                    ...
                    if message.args.len() >= 2 {
                        ...
                        if !message.args[0].chains.is_empty() {
                            let condition =
                                self.execute_message_chain(&message.args[0].chains[0])?;
                            ...
                            if is_true {
                                ...
                                if !message.args[1].chains.is_empty() {
                                    result =
                                        self.execute_message_chain(&message.args[1].chains[0])?;
                                ...
                                if !message.args[2].chains.is_empty() {
                                    result =
                                        self.execute_message_chain(&message.args[2].chains[0])?;
            ...
            let receiver_value = match &message.symbol {
                        ...
                        if let Some(local) = frame.get_local(id.name()) {
                            ...
                            for (i, arg) in message.args.iter().enumerate() {
                                ...
                                if i < message.args.len() - 1 && !arg.chains.is_empty() {
                                    ...
                                    if let Some(param_msg) = arg.chains[0].messages.first() {
                                        if let iowa_parser::Symbol::Identifier(param_id) =
                                            &param_msg.symbol
                                        {
                            ...
                            if !message.args.is_empty() {
                                ...
                                if !last_arg.chains.is_empty() {
                                    ...
                                    // In a real implementation, we'd serialize the AST back to
                                    // source For now, just use
                                    // a placeholder empty body
                                    ...
                                    let method_id = self
                                        .memory
                                        .alloc_method(MethodType::UserDefined(method_data));
                ...
                iowa_parser::Symbol::Number(n) => match n {
    ...
    fn number_add(&self, receiver: ObjectRef, args: Vec<Value>) -> Value {
        ...
        let n2 = match &args[0] {
            Value::Number(n) => *n,
            Value::Object(id) => match self.memory.get_object(*id) {
                Some(obj) => match &obj.data {
                    ObjectData::Number(n) => *n,
                    _ => return Value::Nil,
                },
    ...
    pub fn to_tagged_value(&self, val: &Value) -> u64 {
        match val {
            ...
            Value::Object(id) => self.runtime_encode_reference(*id, value::TAG_OBJECT_REF),
        }
    }
        ...
        match iowa_parser::parse(body_source) {
            Ok((_, chains)) => {
                ...
                let method_data = if use_wasm && !chains.is_empty() {
                    ...
                    let wasm_method = crate::runtime::wasm_runtime::WasmMethod::from_ast(
                        &chains[0],
                        &method_data,
                        self,
                    );
    ...
    pub fn create_method_with_wasm(
        &mut self,
        arg_names: Vec<String>,
        wasm_bytes: Vec<u8>,
    ) -> Option<ObjectRef> {
...
impl Memory {
    ...
    pub fn collect_garbage_with_roots(
        &mut self,
        object_roots: &[ObjectRef],
        value_roots: &[ObjectRef],
    ) {
        ...
        let objects_to_remove: Vec<ObjectRef> = self
            .objects
            .iter()
...
mod tests {
    ...
    fn test_memory_allocation() {
        ...
        let str_id = runtime
            .memory
            .alloc_string(Value::String("hello".to_string()));
    ...
    fn test_runtime_garbage_collection() {
        ...
        // Run garbage collection
        {
            ...
            assert!(
                rt.memory.get_object(nested_id).is_none(),
                "Unreachable object should have been collected"
            );

            // But the object referenced from Lobby should still exist
            assert!(
                rt.memory.get_object(root_obj_id).is_some(),
                "Object referenced from Lobby should still exist"
            );
        }
    }
