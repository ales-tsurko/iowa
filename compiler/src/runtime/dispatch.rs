...
pub fn call_message_dispatch(
    ...
    builder
        .ins()
        .brif(is_nil, forward_block, &[], call_block, &[method]);
    ...
    // Call the method (in a real implementation, this would check method type and dispatch
    // accordingly)
...
fn create_args_array(builder: &mut FunctionBuilder, args: &[CraneliftValue]) -> CraneliftValue {
...
fn call_method(
    ...
    builder
        .ins()
        .brif(is_activation, activation_block, &[], primitive_block, &[]);
...
fn call_runtime_method(
    ...
    sig.params
        .push(cranelift_codegen::ir::AbiParam::new(types::I64)); // method
    sig.params
        .push(cranelift_codegen::ir::AbiParam::new(types::I64)); // receiver
    sig.params
        .push(cranelift_codegen::ir::AbiParam::new(types::I64)); // args
    ...
    let callee = builder.import_function(cranelift_codegen::ir::ExtFuncData {
        name: cranelift_codegen::ir::ExternalName::testcase(RUNTIME_CALL_METHOD), /* Use testcase
                                                                                   * as it accepts
                                                                                   * a &str */
        signature: sig_ref,
        colocated: false,
