...
pub fn check_threshold(builder: &mut FunctionBuilder, allocated_bytes: CraneliftValue) -> CraneliftValue {
...
pub fn mark_object(builder: &mut FunctionBuilder, object: CraneliftValue) {
    ...
    let mark_addr = builder.ins().iadd_imm(
        object,
        layout::gc::MARK_OFFSET as i64
    );
    
    // Load the current mark byte
    let mark_byte = builder.ins().load(
        types::I8,
        MemFlags::trusted(),
        mark_addr,
        0
