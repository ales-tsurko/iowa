...
pub fn alloc_object(
    ...
    let size = builder.ins().iconst(
        types::I32,
        initial_slots as i64 * layout::slots_table::ENTRY_SIZE as i64
...
pub fn alloc_string(builder: &mut FunctionBuilder, length: CraneliftValue) -> CraneliftValue {
    ...
    let header_size = builder
        .ins()
        .iconst(types::I32, layout::string::HEADER_SIZE as i64);
    ...
    builder
        .ins()
        .store(MemFlags::trusted(), zero, ptr, layout::string::HASH_OFFSET);
...
pub fn alloc_number(builder: &mut FunctionBuilder, value: CraneliftValue) -> CraneliftValue {
    let size = builder
        .ins()
        .iconst(types::I32, layout::number::SIZE as i64);
